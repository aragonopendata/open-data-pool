...
/*
$id = 11;
$carpeta = "20180326_151737";

$isql_exe = "isql-vt";
$isql_host = "localhost";
$isql_username = "dba";
$isql_password = "dba";
$isql_db ="http://alfonso.com#";

$email_protocol ="ssl";
$email_host = "smtp.gmail.com";
$email_port  = 465;
$email_username = 'servidotnet@gmail.com';
$email_password = 'adminSDN@';
$email_from = "servidotnet@gmail.com";
$email_to  = "servidotnet@gmail.com";
*/
 ...
 *  email_password:     Conmtraseña cuanta smtp
...
{  
  ...
  $pathError =  sprintf("%s/Error/%s", $webPath, $carpeta);
    ...
    //genera un nuevo contennedor de trazas
  $trazas = new Trazas($pathNoporcesados);
  $trazas->setClase("worker");

  //Si la carpeta no existe infroma del error
  ...
  if ($trazas->SinError()) {
    $trazas->LineaInfo("Procesa","Carga las isonomia y procesa el esquema"); 
  ...
  //Si no hay errores genro las triples
...
//  }
  ...
  if ($trazas->SinError()) {
    $trazas->LineaInfo("Procesa","Mueve la carpeta a la capeta raiz de Procesados");
    $fileSystem->mkdir($pathPorcesados, 0755);
    rename($pathNoporcesados,$pathPorcesados);
  } else {
    $trazas->LineaInfo("Procesa","Mueve la carpeta a la capeta raiz de errores");
 ...
 * Parametros:
 *    pilaTriples:      array contedor de la las triples  (objetos Triple)
 *    regitrosCSV:      array con los datos del archivo CSV
 *    pathNoporcesados: poath de la carpeta donde está el archivo n3 a procesar
...
{    
  $trazas->LineaInfo("GeneraTriples","Inicio de genarción de triples"); 
  ...
  foreach ($regitrosCSV as $filaCVS) 
  {
    foreach ($pilaTriples as $triple) 
    {
        ...
        //la triple pude ser nula porque algun campo del archivo CSV esta vacio
...
/**
 * Función que guarda las triples en servidor Vituoso mediante la herraminata de lines comando isql
 * Parametros:
...
function GuardaTriplesVirtuoso($pathNoporcesados,$trazas,
  ...
  if (file_exists($nombrefichero)) { 
    try{
      //copia el archivo a la ruta  /web/publicacion que gtiene permisos concedidos por virtuosos
    ...
    if ($trazas->SinError()) {
      ...
      //DB.DBA.TTLP (file_to_string_output ('/var/www/AodPool/web/publicacion/datos.n3'), '', 'http://localhost:8890/alfonso/#', 0);
...
/**
 * Función que envia correo al administrador adjuntando el archivi n3 si nio hay error o el de trazas 
 * hay error
 * Parametros:
 *    path:               path de la carpeta donde está el archivo n3 a procesar
 *                        /web/publoicacion/NoProcesados/{$carpeta}
 *    sinError:           Indoca si ha habido error o no
 *    trazas:             Objeto de trazas
 *    email_protocol:     Protocolo ssl
 *    email_host:         Host smtp
 *    email_port:         Puerto smtp
                    ...
                    $email_protocol,$email_host,$email_port,$email_username,$email_password,$email_from,$email_to)
{
    ...
    if ($sinError) {
       $body = "El proceso de publicacion ha terminado con éxito \n. Ajuntamos el archivo de triples generdas";
    } else {
       $body = "El proceso de publicacion ha terminado con érrores \n. Ajuntamos el archivo de log generado";
    }
    // Create a message
