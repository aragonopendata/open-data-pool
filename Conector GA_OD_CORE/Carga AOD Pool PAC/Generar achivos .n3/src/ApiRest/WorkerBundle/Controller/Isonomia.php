 ...
 * cada variable tiene una expresión parseable por sprintf a la espera de los capos del CVS
...
class Isonomia
{
     ...
     */
    public function __construct($id, $trazas, $appPath)
    {
        $this->id = $id;
    ...
    function ProcesaEsquema()
    {
        ...
        // $this->trazas->LineaDebug("ProcesaEsquema",'Isonomia cargada con Identificador:' . $this->id);
        // carga en un DOM xml el texto
        if (empty($isonomia)) {
            // $this->trazas->LineaError("ProcesaEsquema",'Isonomia cargada con Identificador:' . $this->id);
        }
    ...
    /**
     * Funcion que busca el tipo para poner en la entidad es decir de rdf:type ha de devolver type para ponerlo al final del name espace
