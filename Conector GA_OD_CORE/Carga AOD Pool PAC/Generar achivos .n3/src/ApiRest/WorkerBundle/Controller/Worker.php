...
class Worker
{
    ...
    public function __construct($id, $carpeta, $dcType, $actualizarItems, $isqlHost, $isqlDb, $isqlBufferLineas, $smtpEncryption, $smtpHost, $smtpPort, $smtpUsername, $smtpPassword, $emailFrom, $emailTo, $mailFile, $trazasDebug, $usuVirtuoso, $passVirtuoso, $dominioAplicacion, $trazas)
    {
        $this->trazas = $trazas;
        $this->id = $id;
    ...
    public function Procesa($webPath, $appPath)
    {
        ...
        $Csv = new Csv($pathNoporcesados, null);
    ...
    function GeneraTriples($pilaTriples, $registrosCSV, $pathNoporcesados)
    {
        ...
        foreach ($registrosCSV as $filaCVS) {
            ...
            foreach ($pilaTriples as $triple) {
                ...
                if ($triple->getPredicadoValor()=="\"None\"") {
                    //echo 'Valor None encontrado: ' . $triple->getTripleValor() . "\n";
                } else {
    ...
    {
        $this->trazas->LineaInfo("GuardaTriplesVirtuoso", sprintf("%s %s %s %s", $pathNoporcesados, $this->isqlHost, $this->isqlDb, $this->isqlBufferLineas));
    ...
    {
        $trazaTexto = sprintf("path %s email_protocol: %s email_host: %s email_port:%s email_username:%s  email_password:%s  email_from:%s email_to: %s", $path, $this->smtpEncryption, $this->smtpHost, $this->smtpPort, $this->smtpUsername, $this->smtpPassword, $this->emailFrom, $this->emailTo);
        ...
        try {
            ...
            $message->setFrom([
                $this->emailFrom => 'AodPool'
            ]);
            $message->setTo([
                $this->emailTo => 'Administrador AodPool'
            ]);
