<?php
   ...
   if ($archivoCSV !== false) {            
       ...
       array_push ($keys,CLAVE_LABEL); //Le añadimos la clave que necesita y no la tiene el xml
       fwrite ($archivoCSV, "\"".CLAVE_LABEL."\";"); //y la añadidomos al csv
       ...
       for ($i = 1; $i <= $numeroArchivos; $i ++) {
           ...
           for ($x = 0; $x < ($xml->count ()); $x++) {
               foreach ($keys as $key) {
                   ...
                   if ($key == CLAVE_LABEL) {
                       $codSumi = $xml->item[$x]->{OBJECTID}->__toString();
                       $elemento = "Sumidero-$codSumi";
                   }
