<?php 
    ...
    $claveVista60 = "ID_MAN"; //La clave que tiene en comun de las dos vistas
            ...
            for ($x = 0; $x < ($xml->count ()); $x++) {
                foreach ($keys as $key) {
                    ...
                    if ($key == CLAVE_NECESITA) {
                        $codManco = $xml->item[$x]->{CLAVE_TIENE}->__toString ();
                        $codMun = $xmlDepende->xpath ($item."[".$claveVista60."= '".$codManco."']/".CLAVE_NECESITA);
                        $elemento = $codMun [0];
                    }
